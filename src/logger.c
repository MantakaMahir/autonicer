...
int logger_print(const char*p){FILE*f=fopen(p,"r");char line[512];if(!f){if(errno==ENOENT)return 0;perror(p);return -1;}while(fgets(line,sizeof line,f))fputs(line,stdout);fclose(f);return 0;}
