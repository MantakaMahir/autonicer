...
python3 -c 'import json; m=json.load(open("/tmp/autonicer-memory.json")); results=[json.load(open(p)) for p in ("/tmp/autonicer-pager.json","/tmp/autonicer-lru.json","/tmp/autonicer-clock.json")]; assert "availableKb" in m and [x["algorithm"] for x in results] == ["fifo","lru","clock"] and all(x["faults"] > 0 and len(x["steps"]) == 6 for x in results)'
