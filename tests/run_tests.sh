...
python3 -c 'import json; m=json.load(open("/tmp/autonicer-memory.json")); p=json.load(open("/tmp/autonicer-processes.json")); results=[json.load(open(x)) for x in ("/tmp/autonicer-pager.json","/tmp/autonicer-lru.json","/tmp/autonicer-clock.json")]; assert "availableKb" in m and all(x["name"] not in {"systemd","sd-pam","dbus-daemon","pipewire","wireplumber"} for x in p) and [x["algorithm"] for x in results] == ["fifo","lru","clock"] and all(x["faults"] > 0 and len(x["steps"]) == 6 for x in results)'
